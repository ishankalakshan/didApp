...
.controller('weekProgressCtrl',['$scope','didAppDataService',weekProgressCtrl])
...
function weekProgressCtrl($scope,didAppDataService){
  ...
  $scope.$on('didApp.didmobiledata',function(_,result){
    result.feed.entry.forEach(function(b){
      ...
      // });
      $scope.timesheet.push({
          name:b.content.properties.Id.__text
      });
    });
  });
